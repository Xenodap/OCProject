...
def run_machin_learning_rockfish():
    conn = pymysql.connect(host='localhost', user='root', password='1234', db='ocprojectdb', charset='utf8')
    sql = 'select * from future_rockfish'

    df = pd.read_sql_query(sql, conn)
    df.to_csv('C://Bigdata/OCproject/future_rockfish_bigdata.csv', index=False)

    data = pd.read_csv('C://Bigdata/OCproject/future_rockfish_bigdata.csv')
    ...
    path = "C://Bigdata/OCproject/OC_rockfish.pth"
    ...
    print(os.path.join(STATIC_DIR, 'images/bigdata_rockfish.png'))
    fig = plt.figure(figsize=(8, 6))
    ...
    plt.savefig(os.path.join(STATIC_DIR, 'images/bigdata_rockfish.png'))
